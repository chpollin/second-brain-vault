...
This public template supplies reusable structure and synthetic examples. Its static explorer renders the template and explains which files a task requires. The explorer is distinct from the real ACTIVE-WORK interface, which lives in the separate private `active-work-ui` repository, locally checked out as `pruefansicht`, and provides views over selected knowledge and work records.
